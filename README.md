...
| 7 | **首次运行自动初始化** | 新部署时自动拉取本平台现有活动并标记为已同步（不会错误地标记对方平台的活动），避免首次运行产生重复数据 |
...
当你第一次部署（或删除旧 DB 文件重新部署）时，DB 是空的。

> ⚠️ **关键原则**：初始化**只标记本平台的活动**为已同步。对方平台的活动不需要标记——后续运行时的时间重叠比对会自动判断是否需要同步。

```
garmin-coros-sync 初始化（佳明 → 高驰）:
  拉取佳明活动列表 → 标记 is_sync_coros=1（已存在于本平台，无需再同步到高驰）
  → 退出，不上传任何活动
  → 后续运行：佳明活动与高驰有时间重叠？→ 自动跳过 ✅

coros-sync-garmin 初始化（高驰 → 佳明）:
  拉取高驰活动列表 → 标记 is_sync_garmin=1（已存在于本平台，无需再同步到佳明）
  → 退出，不上传任何活动
  → 后续运行：高驰活动与佳明有时间重叠？→ 自动跳过 ✅
```
