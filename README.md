...
> - ✅ 佳明原生活动同步到高驰：高驰 AP 直接读取 FIT 文件中的原始爬升数据，不重新计算
> - ✅ 佳明国内国际互传：同平台，数据格式一致，佳明不做额外纠正
> - ❌ **高驰→佳明**：佳明服务器收到第三方（高驰）的 FIT 文件时，会用自身的 DEM 海拔纠正算法（`elevationCorrected=True`）重新计算每个 GPS 点的海拔，导致爬升被大幅放大。
>
> **为什么高驰→佳明会触发纠正？**
>
> 佳明区分「自家设备」和「第三方」上传的文件。自家设备（佳明手表）录制的活动 `elevationCorrected=False`，直接取用设备数据。第三方 FIT 文件（如高驰）会被重新处理：佳明用全球 DEM（数字高程模型）拟合 GPS 轨迹点的海拔，而 DEM 在丘陵地带的精度远低于实际气压计测量。福州属丘陵地形，DEM 模型的平均海拔与实际地形起伏差异较大。
>
> **实际数据对比**：
