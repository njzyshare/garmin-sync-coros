...
当你第一次部署（或删除旧 DB 文件重新部署）时，DB 是空的。如果直接同步，会把对方平台已有的活动全部传过去，产生大量重复数据。

**初始化流程自动解决这个问题**：工作流首次运行时检测到 DB 为空，会自动拉取双方平台现有的活动列表，全部标记为「已同步」，然后立即退出，不会上传任何活动。

```
garmin_coros.db 为空 → 检测到首次运行
  ↓
拉取佳明活动列表 → 标记已同步
拉取高驰活动列表 → 标记已同步
  ↓
